...
class ContractDeployer:
    ...
    def compile(self, contract_path="Contract_Test.sol"):
        """
        ...
        contract_data = compiled_sol["contracts"]["Contract_Test.sol"]["Contract_Test"]
        ...
        with open("Contract_Test_abi.json", "w") as f:
            json.dump(abi, f, indent=2)

        # Сохранение байткода в файл
        with open("Contract_Test_bytecode.txt", "w") as f:
            f.write(bytecode)

        return abi, bytecode

    def deploy(self, abi, bytecode, initial_value=42):
        """
        ...
        with open("Contract_Test_address.txt", "w") as f:
            f.write(contract_address)

        return contract_address, tx_receipt

    def contract_info(self, contract_address, abi):
        """
...
def main():
    ...
    print("=" * 60)
    print("Деплой смарт-контракта Contract_Test")
    print("=" * 60)
    ...
    try:
        ...
        deployer = ContractDeployer(
            ...
            private_key="23be77bcfbf26025e1fadbf7610df0738ef0aa3e48437075d2ed1a825f92174c"
        )

        # Шаг 1: Компиляция контракта
        abi, bytecode = deployer.compile("Contract_Test.sol")

        # Шаг 2: Деплой контракта
        contract_address, tx_receipt = deployer.deploy(abi, bytecode, initial_value=100)

        # Шаг 3: Получение информации о контракте
        contract = deployer.contract_info(contract_address, abi)

        print("\nДеплой завершен успешно!")
        print(f"   ABI сохранен в: Contract_Test_abi.json")
        print(f"   Байткод сохранен в: Contract_Test_bytecode.txt")
        print(f"   Адрес контракта сохранен в: Contract_Test_address.txt")
