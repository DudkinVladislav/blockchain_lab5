...
class EthereumTransaction:
    ...
    def transfer_ether(self, to_address, value_eth):
        ...
        print(f"\n{'=' * 60}")
        print(f"Перевод {value_eth} ETH на адрес {to_address}")
        print(f"{'=' * 60}")
        ...
        tx_info = self.transaction_info(tx_hash)

        # 5. Получаем информацию о блоке
        if tx_receipt.blockNumber:
            block_info = self.block_info(tx_receipt.blockNumber)
...
def main():
    ...
    try:
        ...
        eth_handler = EthereumTransaction(
        ...
        # Адрес получателя
        to_address = '0xbbd9cfa331e456f2de06b5d983b4a0411b2838f4'
